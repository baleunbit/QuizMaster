...
public class ChatGPTClient : MonoBehaviour
{
    ...
    private IEnumerator RequestQuizQuestions(int count, string topic)
    {
        string prompt = $"다음 조건에 맞는 객관식 퀴즈 문제를 {count}개 생성해주세요:\n" +
                       $"주제: {topic}\n" +
                       "조건:\n" +
                       "- 문제와 보기는 20자 이내로 짧게 작성 해주세요\n" +
                       "- 시간이 6초 남았을 때 힌트를 보여주세요\n" +
                       "- 문제와 보기는 항상 게임 내에 실제로 존재하는 문제와 보기를 구성 해주세요\n" +
                       "- 문제와 보기는 항상 말이 되도록 구성 \n" +
                       "- 선택지는 함정이 있거나 재치있게 구성 해주세요\n" +
                       "- 정답은 0~3 사이의 인덱스로 표시 해주세요\n" +
                       "- 문제와 선택지는 흥미롭고 참여하고 싶게 만들어 주세요\n" +
                       "- 응답은 반드시 다음 JSON 형식으로만 제공 해 주세요:\n" +
                       "{\n" +
                       "  \"questions\": [\n" +
                       "    {\n" +
                       "      \"question\": \"문제 내용\",\n" +
